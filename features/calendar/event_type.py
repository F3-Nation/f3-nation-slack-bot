...
def build_event_type_form(
    ...
    event_types_in_org: List[EventType] = DbManager.find_records(
        EventType,
        [
            EventType.is_active,
            or_(EventType.specific_org_id == region_record.org_id, EventType.specific_org_id.is_(None)),
        ],
    )
...
EVENT_TYPE_FORM = orm.BlockView(
    blocks=[
        ...
        orm.InputBlock(
            ...
            optional=True,
            hint="Tthis is required for national aggregations (achievements, etc).",
        ),
        orm.InputBlock(
