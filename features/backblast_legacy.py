...
from utilities.database.orm.paxminer import Attendance, Backblast, PaxminerAO, PaxminerUser
...
def build_backblast_form(body: dict, client: WebClient, logger: Logger, context: dict, region_record: SlackSettings):
    ...
    if (
        ...
        duplicate_check = safe_get(body, "view", "callback_id") == actions.BACKBLAST_EDIT_CALLBACK_ID_LEGACY
    ...
    if safe_get(body, "actions", 0, "action_id") in [
        ...
        initial_backblast_data = forms.BACKBLAST_LEGACY_FORM.get_selected_values(body)
    elif (safe_get(body, "view", "callback_id") == actions.BACKBLAST_EDIT_CALLBACK_ID_LEGACY) or (
        safe_get(body, "actions", 0, "action_id") == actions.BACKBLAST_EDIT_BUTTON_LEGACY
    ):
    ...
    backblast_form = copy.deepcopy(forms.BACKBLAST_LEGACY_FORM)
...
def handle_backblast_post(body: dict, client: WebClient, logger: Logger, context: dict, region_record: SlackSettings):
    create_or_edit = (
        "create" if safe_get(body, "view", "callback_id") == actions.BACKBLAST_CALLBACK_ID_LEGACY else "edit"
    )
...
*COUNT*: {count}"""
    ...
    edit_block = {
        ...
        "elements": [
            {
                ...
                "value": "edit",
                "action_id": actions.BACKBLAST_EDIT_BUTTON_LEGACY,
            },
            {
                "type": "button",
                "text": {
        ...
        "block_id": actions.BACKBLAST_EDIT_BUTTON_LEGACY,
    }
...
def get_channel_name(id, logger, client, region_record: SlackSettings = None):
    ao_record = None
    if region_record.paxminer_schema:
