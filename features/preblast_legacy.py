...
def build_preblast_form(body: dict, client: WebClient, logger: Logger, context: dict, region_record: SlackSettings):
    ...
    if (safe_get(body, "command") in ["/preblast"]) or (
        ...
        callback_id = actions.PREBLAST_CALLBACK_ID_LEGACY
        ...
        callback_id = actions.PREBLAST_EDIT_CALLBACK_ID_LEGACY
...
def handle_preblast_post(body: dict, client: WebClient, logger: Logger, context: dict, region_record: SlackSettings):
    create_or_edit = (
        "create" if safe_get(body, "view", "callback_id") == actions.PREBLAST_CALLBACK_ID_LEGACY else "edit"
    )

    preblast_data = forms.PREBLAST_LEGACY_FORM.get_selected_values(body)
    ...
    action_block = {
        ...
        "elements": [
            {
                ...
                "value": "edit",
                "action_id": actions.PREBLAST_EDIT_BUTTON_LEGACY,
            },
            {
                "type": "button",
                "text": {
        ...
        "block_id": actions.PREBLAST_EDIT_BUTTON_LEGACY,
    }
